...
public:
    ...
    IMUBridge sBridge;       // 用于传递IMU数据到预测迭代模块
    ...
    // 预测更新用变量
    state_ikfom state_begin;                            // 当前帧初始状态，来自上一次估计
    deque<sensor_msgs::Imu::ConstPtr> q_current_imu;    // 当前帧的IMU数据
    double last_delta_time;                             // 用于计算最后一帧状态量的时间间隔 pcl_end_time - imu_end_time
...
ImuProcess::ImuProcess()
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1), sBridge()
{
